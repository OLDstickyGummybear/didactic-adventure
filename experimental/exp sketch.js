...
function setup() {
  ...
  c = createGraphics(windowWidth, windowHeight, WEBGL);
  m = createGraphics(windowWidth, windowHeight);
  noStroke();
  angleMode(DEGREES);
  noSmooth();
  textAlign(CENTER);
...
function draw() {
  if (isInMenu) {
    ...
    c.background(120, 167, 255);
  

    c.directionalLight(150, 150, 150, 0.1, 0, 0);
    c.directionalLight(100, 100, 100, 0, 0, 0.1);
    c.directionalLight(200, 200, 200, 0, 0, -0.1);
    c.directionalLight(200, 200, 200, -0.1, 0, 0);
    c.directionalLight(255, 255, 255, 0, 0.1, 0);
    ...
    image(c, -width / 2, -height/2, width, height);
  }
    ...
    for (let x = Math.max(round(camXB) - distance, 1); x <= Math.min(round(camXB) + distance, array[0].length - 1); x ++) {
      for (let z = Math.max(round(camZB) - distance, 1); z <= Math.min(round(camZB) + distance, array[0][0].length - 1); z ++) {
        airChecklist = [[array[y+1][x][z] === 0 && y < camYB,     -90,         0,  BLOCKWIDTH/2, 2, 'red'], // down
          ...
          // push();
          c.translate(inCoords(x), inCoords(y), inCoords(z));
          ...
          for (let checkedSide of airChecklist) {
            ...
            if (array[y][x][z] !== 0 && checkedSide[0]) {       
              ...
              c.rotateX(checkedSide[1]);
              c.rotateY(checkedSide[2]);

              c.translate(0, 0, checkedSide[3]);
              c.texture(textureMap.get(blockDict[array[y][x][z]][0])[checkedSide[4]]);

              // fill(checkedSide[5]);
              c.plane(BLOCKWIDTH, BLOCKWIDTH);
...
function moveCam(cam, array) {
  ...
  if (!isInBlock(cam.eyeX, newCamY, cam.eyeZ)) {
    newCamY += camYD;
    if (isInBlock(cam.eyeX, newCamY, cam.eyeZ)) { // if the new height is within a block,
      newCamY = inCoords(inBlocksRound(newCamY)) - 1; // set player height to just one pixel above the block
      // newCamY -= camYD
      isInAir = false;
      camYD = 0;
    } else {
...
function isInBlock(x, y, z) { // inCoords
...
function initiateMenu() {
  testButton = new Button(10, 10, 100, 200, 'Test Button');
...
class Button {
  ...
  display() {
    ...
    if (this.isHovering) {
      m.fill(this.hoverBorderColor);
    } else {
      m.fill(this.defaultBorderColor)
    }
    m.rect(this.x, this.y, this.width, this.height);
    
    // top/left edge
    m.fill(this.color[0])
    // m.triangle(this.x + this.border, this.y + this.border, this.x + this.width - this.border, this.y + this.border, this.x + this.border, this.y + this.height - this.border);
    m.rect(this.x + this.border, this.y + this.border, this.width - 2*this.border - this.edge, this.height - 2*this.border - this.edge);
    m.triangle(this.x + this.width - this.border - this.edge, this.y + this.border, this.x + this.width - this.border, this.y + this.border, this.x + this.width - this.border - this.edge, this.y + this.border + this.edge);


    // bottom/right edge
    m.fill(this.color[2])
    m.triangle(this.x + this.width - this.border, this.y + this.border, this.x + this.width - this.border, this.y + this.height - this.border, this.x + this.border, this.y + this.height - this.border);
    
     // middle
     m.fill(this.color[1])
     m.rect(this.x + this.edge + this.border, this.y + this.edge + this.border, this.width - 2* (this.edge + this.border), this.height - 2* (this.edge + this.border));
  
    // TEXT
    m.fill(255);
    m.text(this.text, this.x + this.width/2, this.y + this.height/2);
